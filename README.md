...
| `/voice_models/upload` | POST | ボイスモデル作成（拡張） |
| `/voice_models/{nickname}` | DELETE | ボイスモデル削除（拡張） |
| `/voice_models/{nickname}/export` | GET | ボイスモデルエクスポート（拡張） |
| `/voice_clone/synthesis` | POST | 参照音声から直接合成（拡張） |
