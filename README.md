...
> **<span style="color:red;">⚠️ 重要: 音声学習に使用するリファレンス音声は、長さが 3秒〜15秒 の範囲内である必要があります。範囲外の音声を使用するとエラーになります。</span>**
...
### 音声合成WebAPIモード（VOICEVOX互換）
- VOICEVOX互換のHTTP APIサーバーとして利用可能
- `GET /speakers`、`POST /audio_query`、`POST /synthesis` などVOICEVOXと同じAPIエンドポイント
- Swagger UI / ReDoc によるAPIドキュメント（`/docs`、`/redoc`）
- **モデルは常に 1.7B を使用**（APIからの変更不可）
- **ボイスモデルはWebUIで学習・保存済みの.ptキャッシュのみ使用可能**
- Temperature などの生成パラメーターはリクエストごとに指定可能
...
### Web APIサーバー（VOICEVOX互換）

```bash
uv run python api_server.py
```

デフォルトで `http://127.0.0.1:50021` でAPIサーバーが起動します。
...
> **注意**: モデルは常に **1.7B** を使用します。ボイスモデルはWebUIで学習・保存済みの.ptキャッシュのみ利用できます。

起動後、以下のURLでAPIドキュメントを参照できます:
...
|----------------|----------|------|
| `/version` | GET | バージョン取得 |
| `/speakers` | GET | 話者（ボイスモデル）一覧 |
| `/speaker_info` | GET | 話者詳細情報 |
| `/audio_query` | POST | 音声合成クエリ作成 |
| `/synthesis` | POST | 音声合成（WAV返却） |
| `/multi_synthesis` | POST | 複数クエリの連結合成 |
| `/voice_models` | GET | ボイスモデル一覧（拡張） |
| `/voice_models/{nickname}/export` | GET | ボイスモデルエクスポート（拡張） |
...
├── api_server.py    # VOICEVOX互換 Web APIサーバー（FastAPI）
